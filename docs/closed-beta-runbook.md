...
| 45 | `sql/45-venue-photo-public-delivery-v1.sql` | **Pending review — do not apply yet:** adds one narrow anonymous `SELECT` policy on `storage.objects` so a visitor can be issued a short-lived signed URL for a venue photo whose row is `visible` and whose venue is published. Keeps the bucket private, grants nothing to `anon` at table or function level, touches no SQL43 or SQL44 policy, and blocks bucket listing through the documented `storage.allow_any_operation(...)` helper plus an exact object-depth check. Run the precheck first: it reports how many images go public on apply and refuses if that helper is unavailable. | 43 |
