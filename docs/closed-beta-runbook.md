...
| 43 | `sql/43-venue-photos-v1.sql` | **Pending review — do not apply yet:** private venue-photo bucket, pending-by-default moderation, owner/admin-only object access, public metadata only for visible photos of published venues, and five authenticated-only guarded RPCs. A future signed-media delivery route must be designed separately; this migration does not make files publicly downloadable. | 23, 35 |
