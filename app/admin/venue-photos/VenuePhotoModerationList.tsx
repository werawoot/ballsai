...
function statusCopy(status: VenuePhotoModerationItem['status']) {
  if (status === 'visible') return { label: 'เผยแพร่แล้ว', tone: '#166534', background: '#ecfdf5', action: 'ซ่อนรูป' }
  if (status === 'hidden') return { label: 'ถูกซ่อน', tone: '#b91c1c', background: '#fff1f1', action: 'อนุมัติและเผยแพร่' }
  return { label: 'รอตรวจสอบ', tone: '#9a3412', background: '#fff7ed', action: 'อนุมัติและเผยแพร่' }
}
  ...
  return <div style={{ display: 'grid', gap: 12 }}>
    ...
    {items.map(item => {
      ...
      return <article key={item.id} style={{ alignItems: 'flex-start', background: '#fff', border: '1px solid #e1e6ec', borderRadius: 13, display: 'flex', flexWrap: 'wrap', gap: 13, padding: 12 }}>
        ...
        <button type="button" disabled={busyId !== null} onClick={() => void moderate(item, nextStatus)} style={{ alignItems: 'center', background: nextStatus === 'visible' ? '#166534' : '#fff8e8', border: nextStatus === 'visible' ? 0 : '1px solid #f0d899', borderRadius: 9, color: nextStatus === 'visible' ? '#fff' : '#854d0e', cursor: busyId ? 'wait' : 'pointer', display: 'inline-flex', fontSize: 12, fontWeight: 900, gap: 6, padding: '9px 11px' }}>
          {busyId === item.id ? <Loader2 size={14} /> : nextStatus === 'visible' ? <Eye size={14} /> : <EyeOff size={14} />}
          {copy.action}
        </button>
      </article>
    })}
