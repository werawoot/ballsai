...
import { Trophy, House, ClipboardList, User, Plus } from 'lucide-react'
  ...
  return (
    <main style={{ background: '#f8f8f8', minHeight: '100vh', paddingBottom: 80, overflowX: 'hidden' }}>
      ...
      <header style={{ position: 'sticky', top: 0, zIndex: 100, display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0 16px', height: 54, background: '#CC0001', boxShadow: '0 2px 12px rgba(204,0,1,0.3)' }}>
        ...
        <div style={{ fontFamily: 'var(--font-oswald)', fontSize: 13, fontWeight: 700, color: 'rgba(255,255,255,0.8)', letterSpacing: 1 }}>ADMIN</div>
      </header>
      ...
      <div style={{ background: '#CC0001', padding: '20px 16px 36px', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative' }}>
          <h1 style={{ fontFamily: 'var(--font-oswald)', fontSize: 'clamp(28px,8vw,48px)', fontWeight: 700, color: 'white', lineHeight: 0.9, textTransform: 'uppercase' }}>
            ADMIN<br />
            <span style={{ WebkitTextStroke: '2px rgba(255,255,255,0.4)', color: 'transparent' }}>PANEL</span>
          </h1>
          <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: 13, marginTop: 10 }}>จัดการ Player Rankings Season 2026</p>
        </div>
      </div>
      ...
      <div style={{ padding: '16px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          ...
          <Link href="/admin/create" style={{ display: 'flex', alignItems: 'center', gap: 4, background: '#CC0001', color: 'white', borderRadius: 20, padding: '6px 14px', fontSize: 12, fontWeight: 800, textDecoration: 'none', fontFamily: 'var(--font-oswald)', letterSpacing: 0.5 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {players?.map((p, i) => (
            <div key={p.id} style={{ background: 'white', borderRadius: 12, border: '1.5px solid #e5e5e5', padding: '14px', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10 }}>
                <div style={{ fontFamily: 'var(--font-oswald)', fontSize: 20, fontWeight: 700, color: '#ccc', width: 28, textAlign: 'center', flexShrink: 0 }}>{i + 1}</div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 15, fontWeight: 800, color: '#111', marginBottom: 3 }}>{p.player_name}</div>
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
                    <span style={{ fontSize: 9, fontWeight: 800, background: '#CC0001', color: 'white', borderRadius: 4, padding: '1px 6px', fontFamily: 'var(--font-barlow)' }}>{p.position}</span>
                    <span style={{ fontSize: 11, color: '#888' }}>{p.team}</span>
                    <span style={{ fontSize: 11, color: '#aaa' }}>{p.province}</span>
                  </div>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 2, flexShrink: 0 }}>
                  <div style={{ fontFamily: 'var(--font-oswald)', fontSize: 20, fontWeight: 700, color: '#CC0001' }}>{p.ovr}</div>
                  <div style={{ fontSize: 10, color: '#aaa', fontWeight: 700 }}>{p.pts.toLocaleString()} pts</div>
                </div>
              </div>

              {/* STATS */}
              <div style={{ display: 'flex', justifyContent: 'space-around', background: '#f8f8f8', borderRadius: 8, padding: '8px', marginBottom: 10 }}>
                {[['PAC', p.pac], ['SHO', p.sho], ['PAS', p.pas], ['DRI', p.dri], ['DEF', p.def]].map(([key, val]) => (
                  <div key={key} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
                    <span style={{ fontFamily: 'var(--font-oswald)', fontSize: 14, fontWeight: 700, color: '#111' }}>{val}</span>
                    <span style={{ fontSize: 9, fontWeight: 700, color: '#aaa', fontFamily: 'var(--font-barlow)' }}>{key}</span>
                  </div>
                ))}
              </div>
