...
export default async function AdminHallPage() {
  ...
  const [{ data: profile }, { data: players }] = await Promise.all([
    ...
    supabase.from('player_ranks').select('id, player_id, player_name, team, province, position').eq('sport', 'football').eq('season', '2026').order('pts', { ascending: false }).limit(500),
  ])
