...
export default function CreatePlayerPage() {
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => {
      ...
      void fetch(`/api/admin/athletes/search?unlinked=1&q=${encodeURIComponent(accountQuery)}`, { signal: controller.signal })
    ...
    <main style={{ background: '#f8f8f8', minHeight: '100vh', paddingBottom: 40 }}>
      ...
      <div style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <div style={{ background: selectedPlayerId ? '#f0fdf4' : 'white', borderRadius: 10, border: `1.5px solid ${selectedPlayerId ? '#15803d' : '#e5e5e5'}`, padding: '18px' }}>
          ...
          <div style={{ alignItems: 'center', background: 'white', border: '1.5px solid #e5e5e5', borderRadius: 10, display: 'flex', gap: 7, marginBottom: 9, padding: '0 11px' }}>
            <Search size={16} color="#66758a" />
            <input value={accountQuery} onChange={event => { setAccountQuery(event.target.value); setSelectedPlayerId('') }} placeholder="ค้นหาชื่อนักกีฬา" aria-label="ค้นหา Athlete Account" style={{ ...inputStyle, background: 'transparent', border: 0, paddingLeft: 0 }} />
          </div>
          ...
          {!loadingAccounts && athleteAccounts.length === 0 && <p style={{ fontSize: 11, color: '#a16207', marginTop: 8 }}>ไม่พบบัญชีที่ยังไม่มี Ranking ลองค้นหาด้วยชื่ออื่น</p>}
          <p style={{ fontSize: 10, color: '#888', lineHeight: 1.5, marginTop: 8 }}>ค้นหาจาก Server ครั้งละไม่เกิน 20 บัญชี และแสดงเฉพาะ Athlete Account ที่ยังไม่มี Ranking ใน {ACTIVE_SEASON}</p>
        </div>
