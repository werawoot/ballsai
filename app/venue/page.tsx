...
export default async function VenueOwnerPage() {
  ...
  const [{ data: venueRows }, { data: bookingRows }, { data: photoRows }] = await Promise.all([
    ...
    // RLS on venue_photos already limits this to the viewer's own venues (or admin);
    // the inner join keeps it to venues this owner owns even for an admin account.
    supabase.from('venue_photos').select('id, venue_id, object_path, caption, sort_order, is_cover, moderation_status, created_at, venue_profiles!inner(owner_id)').eq('venue_profiles.owner_id', user.id).order('sort_order', { ascending: true }),
  ])
