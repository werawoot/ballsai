...
import { Trophy, House, ClipboardList, User, ArrowLeft, Zap, Shield, Star } from 'lucide-react'
import Link from 'next/link'
import { samplePlayerRanks, isSampleId, showDemoData } from '@/lib/sample-data'
...
export default async function PlayerPage({ params }: { params: { id: string } }) {
  ...
  const typedPlayer = player as PlayerRecord
  const cardBg =
    typedPlayer.pts >= 2000
        ...
        <div style={{ position: 'relative', display: 'flex', justifyContent: 'center' }}>
          <div style={{ width: 180, aspectRatio: '2/3', borderRadius: 14, position: 'relative', overflow: 'hidden', background: cardBg, boxShadow: '0 0 0 2px rgba(245,197,24,0.5), 0 20px 60px rgba(0,0,0,0.4)' }}>
            ...
            <div style={{ position: 'absolute', top: 10, left: 10, zIndex: 2 }}>
              <div style={{ fontFamily: 'var(--font-oswald)', fontSize: 36, fontWeight: 800, color: 'rgba(0,0,0,0.75)', lineHeight: 1 }}>{typedPlayer.ovr}</div>
              <div style={{ fontFamily: 'var(--font-barlow)', fontSize: 13, fontWeight: 700, color: 'rgba(0,0,0,0.65)', letterSpacing: 1 }}>{typedPlayer.position}</div>
            </div>
            <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '62%', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1 }}>
              <PositionIcon pos={typedPlayer.position} size={80} />
            </div>
            <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '8px 8px 10px', background: 'linear-gradient(180deg,transparent 0%,rgba(0,0,0,0.7) 30%,rgba(0,0,0,0.88) 100%)', zIndex: 2 }}>
              <div style={{ fontFamily: 'var(--font-barlow)', fontSize: 16, fontWeight: 800, color: 'white', textAlign: 'center', textTransform: 'uppercase', lineHeight: 1.1, marginBottom: 2 }}>{typedPlayer.player_name}</div>
              <div style={{ fontFamily: 'var(--font-barlow)', fontSize: 11, fontWeight: 600, color: 'rgba(255,255,255,0.75)', textAlign: 'center' }}>{typedPlayer.team}</div>
              <div style={{ display: 'flex', justifyContent: 'space-between', gap: 2, marginTop: 10 }}>
                {stats.map((stat) => (
                  <div key={stat.key} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 1 }}>
      ...
      <div style={{ padding: '0 16px', marginTop: -20 }}>
        <div style={{ background: 'white', borderRadius: 16, border: '1.5px solid #e5e5e5', padding: '20px', marginBottom: 12, boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {[
              { label: 'ชื่อ', value: typedPlayer.player_name },
              { label: 'ทีม/สโมสร', value: typedPlayer.team },
              { label: 'จังหวัด', value: typedPlayer.province },
              { label: 'ตำแหน่ง', value: typedPlayer.position },
              { label: 'Power Rating', value: `${typedPlayer.pts.toLocaleString()} Power` },
              { label: 'OVR', value: typedPlayer.ovr },
            ].map((item) => (
            ...
            {stats.map((stat) => (
              <div key={stat.key}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                  <span style={{ fontSize: 12, fontWeight: 700, color: '#555' }}>{stat.label}</span>
                  <span style={{ fontFamily: 'var(--font-oswald)', fontSize: 14, fontWeight: 700, color: '#111' }}>{stat.val}</span>
                </div>
                <div style={{ height: 6, background: '#f0f0f0', borderRadius: 3, overflow: 'hidden' }}>
                  <div style={{ height: '100%', width: `${stat.val}%`, background: stat.val >= 80 ? '#16a34a' : stat.val >= 60 ? '#CC0001' : '#f59e0b', borderRadius: 3, transition: 'width 0.5s ease' }} />
                </div>
              </div>
            ))}
          </div>
